...
class KNearestNeighbor(object):
    ...
    def compute_Cosine_distances_two_loops(self, X):
        ...
        for i in range(num_test):
            for j in range(num_train):
                ...
                #####################################################################
                # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
                dot_product = np.dot(X[i],self.X_train[j])
                x1 = np.sqrt(np.sum(X[i] ** 2))
                x2 = np.sqrt(np.sum(self.X_train[j] ** 2))
                dists[i, j] = 1.0 - (dot_product / (x1 * x2))
                pass
    ...
    def compute_Cosine_distances_one_loop(self, X):
        ...
        for i in range(num_test):
            ...
            x1 = np.sqrt(np.sum(X[i,:] ** 2))
            x2 = np.sqrt(np.sum(self.X_train ** 2, axis=1))
            dists[i,:] = 1.0 - (dot_product / (x1 * x2))
            pass
    ...
    def compute_Cosine_distances_no_loops(self, X):
        ...
        x1 = np.sqrt(np.sum(X ** 2, axis=1, keepdims=True))
        x2 = np.sqrt(np.sum(self.X_train ** 2, axis=1))
        dists = 1 - (dot_product / (x1 * x2))
        pass
