...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        h0, layer0_cache = affine_forward(features, W_proj, b_proj)
        embedded_words, embeddings_cache = word_embedding_forward(captions_in, W_embed)
        h, rnn_cache = rnn_forward(embedded_words, h0, Wx, Wh, b)
        temporal_out, temporal_b = temporal_affine_forward(h, W_vocab, b_vocab)
        loss, dout = temporal_softmax_loss(temporal_out, captions_out, mask)


        dout, grads['W_vocab'], grads['b_vocab'] = temporal_affine_backward(dout, temporal_b)
        dout, dh0, grads['Wx'], grads['Wh'], grads['b'] = rnn_backward(dout, rnn_cache)
        grads['W_embed'] = word_embedding_backward(dout, embeddings_cache)
        dout, grads['W_proj'], grads['b_proj'] = affine_backward(dh0, layer0_cache)
    ...
    def sample(self, features, max_length=30):
        ...
        h_c, _ = affine_forward(features, W_proj, b_proj)
        for i in range(max_length):
            h_c, _ = rnn_step_forward(word_embed, h_c, Wx, Wh, b)
            scores, _ = affine_forward(h_c, W_vocab, b_vocab)
