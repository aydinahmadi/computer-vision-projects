...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
        prev_h, cache_[i] = rnn_step_forward(x[:, i, :], prev_h, Wx, Wh, b)
        h[:, i, :] = prev_h

    cache = cache_, (N, T, D, H)
...
def rnn_backward(dh, cache):
    ...
    for i in reversed(range(T)):
        dx_, dprev_h_, dWx_, dWh_, db_ = rnn_step_backward(dh[:, i, :] + dprev_h_, cache_[i])
        dx[:, i, :] = dx_
        dWx += dWx_
        dWh += dWh_
        db += db_
