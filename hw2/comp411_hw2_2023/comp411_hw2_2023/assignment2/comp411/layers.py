...
def batchnorm_backward_alt(dout, cache):
    ...
    gamma, x, mean, var, eps, normalized_x = cache
    N = x.shape[0]

    stddev = np.sqrt(var + eps)
    dnormalized_x = dout * gamma
    dvar = np.sum((x - mean) * dnormalized_x * -0.5 / stddev ** 3, axis=0)
    dmean = np.sum(- dnormalized_x / stddev, axis=0) + dvar * np.sum(-2 * (x - mean), axis=0)
    dx = dnormalized_x / stddev + dvar * 2 * (x - mean) / N + dmean / N
